...
/**
 * A simple utility class that permits the loading, and in-memory storage, of a given certificate
 * that adheres to X.509 format.
 */
public class SimpleCertManagementUtil {
  ...
  /**
   * Attempts to load the certificate contained in 'fileName' using the Java {@link KeyStore} and
   * {@link TrustManagerFactory} functionality.
   *
   * <p>This will invoke the {@link #loadCertificate(String, String, String} method using the
   * default certificate type of {@link #DEFAULT_CERTIFICATE_TYPE} and default socket type of {@link
   * #DEFAULT_SECURE_SOCKET_TYPE}.
   *
   * <p>On success, the certificate will be loaded by the system, stored under a unique ID, and
   * mapped to an {@link SSLSocketFactory}, which is returned to the caller.
   *
   * <p>On failure, an exception will be logged, and null will be returned.
   *
   * @param fileName The certificate file name to load.
   * @return SSLSocketFactory The socket factory initialized with the loaded certificate.
   */
   ...
   * @param certType The certificate type to load (e.g. "X.509"). If null or otherwise invalid (e.g.
   *     empty), will use the default {@link #DEFAULT_CERTIFICATE_TYPE}.
   * @param socksType The socket type to initialize (e.g. "SSL"). If null or otherwise invalid (e.g.
   *     empty), will use the default {@link #DEFAULT_SECURE_SOCKET_TYPE}.
   * @return SSLSocketFactory The socket factory initialized with the loaded certificate.
   */
  private SSLSocketFactory loadCertificate(
      String serverCrtFileName, String certType, String socksType) {
    ...
    try {
      ...
      _Logger.info(
          certType
              + " certificate load and "
              + socksType
              + " socket init successful from file: "
              + serverCrtFileName);
  ...
  /**
   * Attempts to import the given certificate file and store as a uniquely named keystore reference
   * (based on the filename and available bytes.
   *
   * @param fileName The file name of the certificate to process.
   * @param certType The certificate type to load (e.g. X.509).
   * @return KeyStore A reference to the {@link KeyStore} containing the certificate.
   * @throws KeyStoreException
   * @throws NoSuchAlgorithmException
   * @throws CertificateException
   * @throws IOException If an IO exception occurs, or if the file is available, but has 0 bytes to
   *     read.
   */
  private KeyStore importCertificate(String fileName, String certType)
    ...
    try {
      ...
      if (bis.available() == 0) {
        _Logger.warning(
            "No bytes available. Failed to import " + certType + " from file: " + fileName);
  ...
  /**
   * Checks is the given file name is valid and available on the local file system.
   *
   * @param fileName
