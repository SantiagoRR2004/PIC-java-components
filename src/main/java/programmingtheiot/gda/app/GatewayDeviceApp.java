 ...
 *
 * <p>You may find it more helpful to your design to adjust the functionality, constants and
 * interfaces (if there are any) provided within in order to meet the needs of your specific
...
public class GatewayDeviceApp {
  ...
  public static void main(String[] args) {
    ...
    boolean runForever =
        ConfigUtil.getInstance()
            .getBoolean(ConfigConst.GATEWAY_DEVICE, ConfigConst.ENABLE_RUN_FOREVER_KEY);
  ...
  public void startApp() {
    ...
    try {
      if (!ConfigUtil.getInstance()
          .getBoolean(ConfigConst.GATEWAY_DEVICE, ConfigConst.TEST_EMPTY_APP_KEY)) {
   ...
   *
   * <p>NOTE: This will be added later.
   *
  ...
  private void initConfig(String configFile) {
    _Logger.log(
        Level.INFO,
        "Attempting to load configuration: {0}",
   ...
   *
   * <p>This method should be written to check if any valid command line args are provided,
   * including the name of the config file. Once parsed, call {@link #initConfig(String)} with the
   * name of the config file, or null if the default should be used.
   *
   * <p>If any command line args conflict with the config file, the config file in-memory content
   * should be overridden with the command line argument(s).
   *
