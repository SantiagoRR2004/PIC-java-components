...
public interface IPersistenceClient {
  ...
  /**
   * Attempts to retrieve the named data instance from the persistence server. Will return null if
   * there's no data matching the given type with the given parameters.
   *
  ...
  /**
   * Registers the specified {@link
   * programmingtheiot.gda.connection.labbenchstudios.data.IPersistenceListener} for all {@link
   * #storeData()} calls for the named data type.
   *
   * @param cType The data type to use for storage notifications.
   * @param listener The listener to notify when data is added.
   * @param topics The topic name(s) (comma-delimited) to use for notification purposes. if null,
   *     all store requests for any topic will be used to notify the listener.
   */
  public void registerDataStorageListener(
      Class cType, IPersistenceListener listener, String... topics);
