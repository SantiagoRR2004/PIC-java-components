...
/**
 * Base class for common properties and helper methods for all anticipated parameters and actions
 * shared amongst actuator and sensor data structures within the context of this project.
 */
public abstract class BaseIotData implements Serializable {
   ...
   * Default.
   *
   * <p>Initializes the location ID from the config file and updates the timestamp.
   */
  protected BaseIotData() {
    ...
    String locID =
        ConfigUtil.getInstance()
            .getProperty(ConfigConst.GATEWAY_DEVICE, ConfigConst.DEVICE_LOCATION_ID_KEY);
  ...
  /**
   * Returns the timestamp as a formatted String, based on the millis since the Epoch, which is set
   * via the {@see #updateTimeStamp()} method.
   *
   * <p>The timestamp is represented as a String in ISO 8601 format, as follows:
   *
   * <p>e.g. 2020-12-27T10:13:30Z
   *
  ...
  /**
   * Returns the type ID, which can be used as a convenient way to switch through different IoT data
   * types when processing sub-class instances of this base class.
   *
   * @return int
   */
  ...
  /**
   * Sets the elevation float value. No validation is performed - can be any float value permitted
   * by the JVM.
   *
  ...
  /**
   * Sets the latitude float value. No validation is performed - can be any float value permitted by
   * the JVM.
   *
  ...
  /**
   * Sets the longitude float value. No validation is performed - can be any float value permitted
   * by the JVM.
   *
  ...
  /**
   * Sets the location ID as a string. This can be used as a user-friendly label to identify a
   * location or specialized name inferring a location.
   *
   * <p>Validation rules will check if the ID is non-null and non-empty (post-trim). If the
   * validation fails, nothing is done.
   *
  ...
  /**
   * Sets the name as a string. This can be used as a user-friendly label to identify the IoT data
   * name or device name. It's usually used as the former.
   *
   * <p>Validation rules will check if the name is non-null and non-empty (post-trim). If the
   * validation fails, nothing is done.
   *
  ...
  /**
   * Sets the status code int value. No validation is performed - can be any int value permitted by
   * the JVM.
   *
   * <p>While any allowable int can be passed in as the parameter, internal rules will expect a < 0
   * value to infer an error. This can be changed easily enough by changing the logic in {@see
   * #hasError()}.
   *
  ...
  /**
   * Sets the type ID int value. No validation is performed - can be any int value permitted by the
   * JVM.
   *
  ...
  /**
   * This is used to apply the properties of the 'data' parameter to the internal structure. Only
   * basic validation is performed - that is, if the 'data' parameter is null, nothing is done.
   * Otherwise, all properties - except the timestamp - will be copied over 'as-is' to the internal
   * class-scoped variables using their respective setter methods.
   *
   * <p>Timestamp is generated via a call to {@see #updateTimeStamp()}.
   *
   * <p>This will also trigger a call to the template method {@see #handleUpdateData(BaseIotData)},
   ...
   *
   * @param BaseIotData While the parameter must implement this method, the sub-class is expected to
   *     cast the base class to its given type.
   */
  protected abstract void handleUpdateData(BaseIotData data);

  /**
   * Stores the latest timestamp in milliseconds (since the Epoch) within {@see #timeStampMillis},
   * then sets the internal time stamp string {@see #timeStamp} to represent a new Date based on
   * {@see #timeStampMillis} in ISO 8601 format using {@see DateTimeFormatter.ISO_INSTANT}, as
   * follows:
   *
   * <p>e.g. 2020-12-27T10:13:30Z
   */
