...
/**
 * This test case class contains very basic integration tests for MqttClientPerformanceTest. It
 * should not be considered complete, but serve as a starting point for the student implementing
 * additional functionality within their Programming the IoT environment.
 *
...
public class MqttClientPerformanceTest {
  ...
  private void execTestPublish(int maxTestRuns, int qos) {
    ...
    String msg =
        String.format(
            "\\n"
                + "\\tTesting Publish: QoS = %s | msgs = %s | payload size = %s | start = %s | end"
                + " = %s | elapsed = %s",
            qos,
            maxTestRuns,
            payloadLen,
            (float) startMillis / 1000,
            (float) endMillis / 1000,
